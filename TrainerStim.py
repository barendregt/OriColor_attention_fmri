...
class TrainerStim(object):
	...
	def setup_cue(self):
		...
		if self.trial_settings[self.session.standard_parameters['cue_index_level_' + str(self.session.run_level)]] == 0:
			self.cue_sound = 'red45'
		elif self.trial_settings[self.session.standard_parameters['cue_index_level_' + str(self.session.run_level)]] == 1:
			self.cue_sound = 'red135'
		elif self.trial_settings[self.session.standard_parameters['cue_index_level_' + str(self.session.run_level)]] == 2:
			self.cue_sound = 'green45'
		elif self.trial_settings[self.session.standard_parameters['cue_index_level_' + str(self.session.run_level)]] == 3:
			self.cue_sound = 'green135'

		if self.trial_settings[self.session.standard_parameters['task_index_level_' + str(self.session.run_level)]] == 1:
			taskMessage = 'K'
		else:
			taskMessage = 'O'
