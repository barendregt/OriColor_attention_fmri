...
class TrainerSession(EyelinkSession):
	...
	def prepare_trials_0(self):
		...
		trial_params = np.vstack([trial_params, trial_params])
	...
	def run(self):
		...
		self.phase_durations = np.array([
			self.standard_parameters['trainer_level_'+str(self.run_level)+'_timing_ITI_duration'], 	# inter trial interval
			...
			self.standard_parameters['trainer_level_'+str(self.run_level)+'_timing_stim_1_Duration']		
			])	# ITI	
		...
		this_instruction_string = self.standard_parameters['trainer_level_' + str(self.run_level) + '_task_instruction']
		...
		for self.trialID in range(rStart, len(self.trial_array)):
			...
			if (self.run_level==1) and (self.trialID == self.standard_parameters['trainer_second_level_minimal_trials']/2):

				self.correct_responses = 0

			if self.run_level == 1:

				print 'Performance level: %f' % (100*(self.correct_responses/(1+self.trialID)))

				if (self.trialID > self.standard_parameters['trainer_second_level_minimal_trials']) and ((self.correct_responses/(1+self.trialID)) >= self.standard_parameters['trainer_second_level_pc_goal']):
					break
