			...
			if len(ev) > 0:
				...
				# it handles both numeric and lettering modes 
					...
					if self.phase == self.session.standard_parameters['trainer_response_phase']:
						...
						print 'Cue: %d, response: %d (%s)' % (self.parameters[self.session.standard_parameters['cue_index_level_1']],self.session.standard_parameters['trainer_response_buttons'].index(ev), ev)

						if self.parameters[self.session.standard_parameters['cue_index_level_1']] == self.session.standard_parameters['trainer_response_buttons'].index(ev):
